...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const response = await fetch(
            `http://10.128.0.50:3000/api/exam/get?registerNumber=${registerNumber}`
        );
...
runButton.addEventListener("click", async () => {
    ...
    try {
        const response = await fetch("http://10.128.0.50:3001/run-code", {
            method: "POST",
    ...
    try {
        ...
        if (result.output) {
            ...
            if (result.output.includes("input()") || 
                ...
                const inputResponse = await fetch("http://10.128.0.50:3001/run-code", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ 
                        language, 
