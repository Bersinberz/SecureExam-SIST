...
async function fetchStudentDetails() {
  try {
    ...
    const response = await fetch(
      `http://10.128.0.50:3000/api/students?department=${department}&section=${section}`
    );
...
async function fetchCodeSubmissions() {
  try {
    const response = await fetch("http://10.128.0.50:3000/api/all-codes");
...
async function finishExam() {
  ...
  setTimeout(async function () {
    ...
    try {
      const response = await fetch("http://10.128.0.50:3000/api/exam/finish", {
        method: "POST",
        headers: {
