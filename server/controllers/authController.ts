...
const handleStudentLogin = async (identifier: string, password: string, res: Response) => {
  try {
    ...
    // Atomically pop a question from the pool to avoid race conditions
    const updatedExam = await ExamModel.findOneAndUpdate(
      { _id: currentExam._id, questions: { $exists: true, $not: { $size: 0 } } },
      { $pop: { questions: -1 } }, // removes first element
      { new: false } // return the doc BEFORE the update so we can read the removed question
    );
